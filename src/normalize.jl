...
Normalize grouped columns of `X` in place by applying the same normalization
function to all columns generated by the same feature/transform.

# Arguments
- `X::AbstractArray{<:AbstractFloat}`: Tabular dataset (observations × features).
- `n::Base.Callable`: Normalization constructor (e.g. `zscore()`, `minmax()`); it
  is called once per feature group with the concatenated column values.
